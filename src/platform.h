...
/**
 * Implementation of node-api platform as an interface
 */
struct pomelo_platform_s {
    /// @brief Set the extra data
    void (*set_extra)(pomelo_platform_t * platform, void * extra);

    /// @brief Get the extra data
    void * (*get_extra)(pomelo_platform_t * platform);

    /// @brief Destroy the platform
    void (*destroy)(pomelo_platform_t * platform);

    /// @brief The startup function
    void (*startup)(pomelo_platform_t * platform);
    ...
    uint64_t (*hrtime)(pomelo_platform_t * platform);

    /// @brief The now function
    uint64_t (*now)(pomelo_platform_t * platform);
    ...
    napi_value (*statistic)(pomelo_platform_t * platform, napi_env env);
};
